...
# Reproducibility Checklist (Végső Leadás Előtt)
...
- [ ] [cite_start]**Egygombos indítás:** A fő kísérlet (pl. a 13. heti sweep) egyetlen, a README-ben világosan leírt paranccsal indítható (pl. `python experiments/run_sweep.py`)[cite: 151].
...
- [ ] [cite_start]**Központi Seed rögzítése:** Minden sztochasztikus folyamat (csatorna zaj, backoff, anomália generálás) egy fix, előre megadott seed-del inicializálódik (pl. `np.random.default_rng(42)`)[cite: 149].
...
- [ ] [cite_start]**Config Dump (Paraméterek mentése):** A kísérlet futtatása során a szkript automatikusan kimenti a használt paramétereket és sweep tartományokat egy `config_dump.json` (vagy YAML) fájlba[cite: 150].
- [ ] **Verziókövetés:** A futtatott kód egy adott, címkézett Git commit-hoz (pl. `tag: m4-final`) tartozik.

## 4. Adatok és Kimenetek
- [ ] [cite_start]**Nyers eredmények mentése:** A paraméter-söprés összes futásának eredménye kimentésre kerül egy `reports/data/sweep_results.csv` fájlba, a Pareto-szűrés előtt[cite: 150].
- [ ] [cite_start]**Automata ábragenerálás:** Az ábrák automatikusan elmentődnek a `reports/figures/` mappába[cite: 150].
